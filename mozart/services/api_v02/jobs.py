...
job_ns = Namespace("job", description="Mozart job operations")
queue_ns = Namespace("queue", description="Mozart queue operations")
on_demand_ns = Namespace(
    "on-demand", description="For retrieving and submitting on-demand jobs for mozart"
)
...
@job_ns.doc(
    responses={200: "Success", 400: "Invalid parameters", 500: "Job submission failed"},
    description="Submit job for execution in HySDS.",
)
class SubmitJob(Resource):
    """Submit job for execution in HySDS."""

    resp_model = job_ns.model(
        "SubmitJobResponse",
        {
            "success": fields.Boolean(
                required=True, description="Boolean, whether the API was successful"
            ),
    ...
    parser.add_argument(
        "type", required=True, type=str, help="a job type from jobspec/list"
    )
    parser.add_argument(
        "queue",
        required=True,
    ...
    parser.add_argument(
        "priority", required=False, type=int, help="Job priority in the range of 0 to 9"
    )
    parser.add_argument(
        "tags",
        required=False,
    ...
    parser.add_argument(
        ...
        required=False,
        type=str,
        help="base job name override; defaults to job type",
    )
    parser.add_argument(
        "payload_hash", required=False, type=str, help="user-generated payload hash"
    )
    ...
    parser.add_argument(
        ...
        required=False,
        type=bool,
        help="flag to enable/disable job dedup",
    )
    parser.add_argument(
        "soft_time_limit", type=str, help="soft time limit for job execution"
    )
    parser.add_argument(
        "time_limit", type=str, help="hard time limit for job execution"
    )
    parser.add_argument(
        "disk_usage", type=str, help="disk usage for PGE (KB, MB, GB, etc)"
    )
    parser.add_argument(
        "publish_overwrite_ok",
        type=bool,
        help="enable a job to overwrite an existing dataset on publish",
    )
    parser.add_argument(
        "params",
        required=False,
    ...
    def post(self):
        ...
        enable_dedup = str(
            request.form.get("enable_dedup", request.args.get("enable_dedup", "true"))
        )

        soft_time_limit = request.form.get(
            "soft_time_limit", request.args.get("soft_time_limit", None)
        )
        time_limit = request.form.get(
            "time_limit", request.args.get("time_limit", None)
        )
        disk_usage = request.form.get(
            "disk_usage", request.args.get("disk_usage", None)
        )

        publish_overwrite_ok = request.form.get(
            "publish_overwrite_ok", request.args.get("publish_overwrite_ok", False)
        )
        ...
        try:
            if enable_dedup.strip().lower() == "true":
                ...
                raise Exception(
                    f"Invalid value for param 'enable_dedup': {enable_dedup}"
                )
            ...
            try:
                ...
                raise Exception(f"Failed to parse input tags. '{tags}' is malformed")
            ...
            try:
                ...
                raise Exception(
                    f"Failed to parse input params. '{params}' is malformed"
                )

            app.logger.warning(job_type)
            app.logger.warning(job_queue)
            job_json = hysds_commons.job_utils.resolve_hysds_job(
                job_type,
                job_queue,
                priority,
                tags,
            ...
            message = f"Failed to submit job. {type(e)}:{str(e)}"
            app.logger.error(message)
            return {"success": False, "message": message}, 500
        return {"success": True, "message": "", "result": ident, "tags": tags}


@job_ns.route("/user/<user>", endpoint="user-jobs")
@job_ns.doc(
    responses={200: "Success", 500: "Query execution failed"},
    description="Get list of user submitted job IDs",
)
class UserJobs(Resource):
    ...
    parser.add_argument(
        "type", type=str, help="job type + version (ie. topsapp:v1.0)", required=False
    )
    ...
    parser.add_argument(
        "start_time", type=str, help="start time of @timestamp field", required=False
    )
    parser.add_argument(
        "end_time", type=str, help="start time of @timestamp field", required=False
    )
    parser.add_argument(
        "status",
        type=str,
        help="job status, ie. job-queued, job-started, job-completed, " "job-failed",
        required=False,
    )
    ...
    def get(self, user):
        ...
        query = {
            ...
            "query": {"bool": {"must": [{"term": {"job.username": user}}]}},
        }
        ...
        if queue:
            query["query"]["bool"]["must"].append(
                {"term": {"job.job_info.job_queue": queue}}
            )
        ...
        return {
            "success": True,
            "result": [
                {"id": doc["_id"], "tags": doc["_source"]["tags"]}
...
class JobQueues(Resource):
    ...
    resp_model = queue_ns.model(
        "Queue Listing Response(JSON)",
        {
            "success": fields.Boolean(
                required=True, description="Boolean, whether the API was successful"
            ),
    ...
    parser.add_argument(
        "id", required=False, type=str, help="Job Type Specification ID"
    )
...
class JobStatus(Resource):
    ...
    resp_model = job_ns.model(
        "Job Status Response(JSON)",
        {
            ...
            "status": fields.String(
                required=True, enum=job_statuses, description="job status"
            ),
        },
    )
    ...
    def get(self, _id=None):
        ...
        job_status = mozart_es.search_by_id(
            index=JOB_STATUS_INDEX, id=_id, ignore=404, _source=["status"]
        )
        if job_status["found"] is False:
            return {"success": False, "message": "job status not found: %s" % _id}, 404
...
class Jobs(Resource):
    ...
    resp_model = job_ns.model(
        "Jobs Listing Response(JSON)",
        {
            ...
            "result": fields.List(
                fields.String, required=True, description="list of job IDs"
            ),
        },
    )
    parser = job_ns.parser()
    parser.add_argument(
        "page_size", required=False, type=str, help="Job Listing Pagination Size"
    )
    parser.add_argument(
        "offset", required=False, type=str, help="Job Listing Pagination Offset"
    )
...
class JobInfo(Resource):
    ...
    def get(self, _id=None):
        ...
        if info["found"] is False:
            return {"success": False, "message": "job info not found: %s" % _id}, 404

        return {"success": True, "message": "", "result": info["_source"]}


@on_demand_ns.route("", endpoint="on-demand")
@on_demand_ns.doc(
    responses={200: "Success", 500: "Execution failed"},
    description="Retrieve/submit on demand jobs",
)
class OnDemandJobs(Resource):
    """On Demand Jobs API."""

    resp_model = on_demand_ns.model(
        "JsonResponse",
        {
            "success": fields.Boolean(
                required=True, description="if request was successful"
            ),
            "message": fields.String(
                required=True, description="message describing success or failure"
            ),
    ...
    parser.add_argument(
        "job_type", type=str, location="form", required=True, help="job spec name"
    )
    parser.add_argument(
        "hysds_io", type=str, location="form", required=True, help="hysds io name"
    )
    ...
    def get(self):
        ...
        query = {
            ...
            "query": {"exists": {"field": "job-specification"}},
        }
    ...
    def post(self):
        ...
        try:
            ...
            app.logger.error(e)
            return {"success": False, "message": "invalid JSON query"}, 400
...
class JobParams(Resource):
    ...
    resp_model = on_demand_ns.model(
        ...
        {
            "success": fields.Boolean(
                required=True, description="if request was processed successfully"
            ),
            "message": fields.String(
                required=True, description="message describing success or failure"
            ),
            "objectid": fields.String(
                required=True, description="ID of indexed dataset"
            ),
    ...
    def get(self):
        ...
        query = {"query": {"term": {"job-specification.keyword": job_type}}}

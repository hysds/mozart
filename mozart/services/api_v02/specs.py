...
class JobSpecs(Resource):
    ...
    resp_model_job_spec = job_spec_ns.model(
        "Job Specification List Response(JSON)",
        {
    ...
    post_job_spec_parser.add_argument(
...
@container_ns.doc(
    responses={200: "Success", 500: "Query execution failed"},
    description="Containers endpoint",
)
class Containers(Resource):
    ...
    resp_model = container_ns.model(
        "Job Info Response(JSON)",
        {
            ...
            "result": fields.Raw(
                required=True, description="Container information or ID"
            ),
        },
    )
    ...
    def delete(self):
        ...
        return {"success": True, "message": "job_spec deleted: %s" % _id, "result": _id}
...
class HySDSIOTypes(Resource):
    ...
    resp_model_job_types = hysds_io_ns.model(
...
class HySDSio(Resource):
    ...
    post_parser.add_argument(
        "spec", required=True, type=dict, help="HySDS IO JSON Object"
    )

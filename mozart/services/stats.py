...
def parse_cpu_stats(output):
    ...
    for line in output.split("\n"):
        ...
        if stats["cpu"] == "CPU":
            ...
            values.extend(
                [
                    {"label": "all CPU (user)", "value": stats["user"]},
                    {"label": "all CPU (sys)", "value": stats["sys"]},
                ]
            )
        else:
            values.extend(
                [
                    {"label": "CPU %s (user)" % stats["cpu"], "value": stats["user"]},
                    {"label": "CPU %s (sys)" % stats["cpu"], "value": stats["sys"]},
                ]
            )
    return values
...
def parse_mem_stats(output):
    ...
    for line in output.split("\n"):
        ...
        if match:
            ...
            if mem_total == 0.0:
                ...
                mem_used_perc = math.ceil(
                    float(stats["used"]) / float(stats["total"]) * 100.0
                )
            values.append({"label": "memory", "value": mem_used_perc})
            continue
        ...
        if match:
            ...
            values.append({"label": "swap", "value": swap_used_perc})
            continue
    return values
...
def execute_nodes():
    ...
    return jsonify(
        {"success": True, "message": "", "nodes": nodes, "total": len(nodes)}
    )
...
def get_mpstat():
    ...
    if node is None:
        return jsonify({"success": False, "message": "No execute node specfied."}), 500
    ...
    try:
        ...
        app.logger.info(
            "Failed to execute mpstat('%s', '%s'):\n%s\n%s"
            % (user, node, str(e), traceback.format_exc())
        )
        return jsonify(
            {"success": False, "message": str(e), "stats": None, "node": node}
        )

    return jsonify({"success": True, "message": "", "stats": output, "node": node})
...
def get_mem_free():
    ...
    try:
        ...
        app.logger.info(
            "Failed to execute mem_free('%s', '%s'):\n%s\n%s"
            % (user, node, str(e), traceback.format_exc())
        )
        return jsonify(
            {"success": False, "message": str(e), "stats": None, "node": node}
...
def get_cpu_mem_stats():
    ...
    try:
        ...
        app.logger.info(
            "Failed to execute mpstat/mem_free('%s', '%s'):\n%s\n%s"
            % (user, node, str(e), traceback.format_exc())
        )
        return jsonify(
            {
                ...
                "node": node,
                "date": None,
                "stats": {"cpu": None, "memory": None, "top": None},
                "data": [],
            }
        )
    ...
    return jsonify(
        {
            ...
            "node": node,
            "date": date,
            "stats": {"cpu": cpu_stats, "memory": mem_stats, "top": top_procs},
            "data": [{"key": "CPU and Memory Utilization", "values": values}],
        }
    )

...
class GetJobTypes(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            message = "Failed to query ES for Job types. {0}:{1}".format(type(e),str(e))
...
class AddJobSpecType(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            message = "Failed to add ES for Job spec. {0}:{1}".format(type(e),str(e))
            app.logger.warning(message)
            app.logger.warning(traceback.format_exc(e))
            return {'success': False, 'message': message}, 500
        return { 'success': True,
...
class SubmitJob(Resource):
    ...
    parser.add_argument('priority', required=False, type=int, help='Job priority in the range of 0 to 9')
    ...
    def post(self):
        ...
        try:
            ...
            if enable_dedup.strip().lower() == "true": enable_dedup = True
            elif enable_dedup.strip().lower() == "false": enable_dedup = False
            else: raise Exception("Invalid value for param 'enable_dedup': {0}".format(enable_dedup))
            try:
                if not tags is None:
...
class GetContainerTypes(Resource):
    ...
    def get(self):
        ...
        '''
        try:
            ids = hysds_commons.container_utils.get_container_types(app.config['ES_URL'], logger=app.logger)
        except Exception as e:
            message = "Failed to query ES for container types. {0}:{1}".format(type(e),str(e))
            app.logger.warning(message)
            app.logger.warning(traceback.format_exc(e))
            return {'success': False, 'message': message}, 500
        return { 'success': True,
...
class GetContainerRemove(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            message = "Failed to remove container {2}. {0}:{1}".format(type(e),str(e),ident)
            app.logger.warning(message)
            app.logger.warning(traceback.format_exc(e))
            return {'success': False, 'message': message}, 500
        return { 'success': True,

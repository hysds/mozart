...
def process_this():
    ...
    rule_temp = {"rule_name": rule["rule_name"], "queue": app.config.get(
        "JOB_SUBMISSION_QUEUE", rule["queue"]), "priority": rule["priority"], "kwargs": "{}"}
    param_temp = [{"name": "submitter", "from": "value", "value": "mozart"}, {
        "name": "submitter_rule", "from": "value", "value": json.dumps(rule)}]

    hysds_commons.job_rest_utils.single_process_and_submission(app.config["MOZART_URL"], {}, rule_temp, hysdsio={
                                                               "id": "internal-temporary-wiring", "params": param_temp, "job-specification": app.config["JOB_SUBMISSION_JOB_SPEC"]})

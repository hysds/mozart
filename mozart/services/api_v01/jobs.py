...
on_demand_ns = Namespace(
    ON_DEMAND_NS, description="For retrieving and submitting on-demand jobs for mozart"
)
...
class SubmitJob(Resource):
    ...
    parser.add_argument(
        "priority", type=int, help="Job priority in the range of 0 to 9"
    )
    parser.add_argument(
        "tags", type=str, help='JSON list of tags, e.g. ["dumby", "test_job"]'
    )
    parser.add_argument(
        "name", type=str, help="base job name override; defaults to job type"
    )
    ...
    parser.add_argument(
        "enable_dedup", type=bool, help="flag to enable/disable job dedup"
    )
    parser.add_argument(
...
class ProductsStaged(Resource):
    def get(self, _id):
        ...
        prod = mozart_es.search_by_id(
            index=JOB_STATUS_INDEX, id=_id, _source_includes=doc_fields, ignore=404
        )
        app.logger.info("fetch products staged for %s" % _id)

        if prod["found"] is False:
            return {"success": False, "message": "Job id not found: %s" % _id}, 404

        doc = prod["_source"]
        status = doc["status"]
        if status not in {"job-failed", "job-completed"}:
            return {"success": False, "message": "job has not been completed"}
        try:
            return {
                "success": True,
                "message": status,
            ...
            return {"success": True, "message": status, "results": []}

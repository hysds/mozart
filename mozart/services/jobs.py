...
def job_count():
    ...
    body = {"size": 0, "aggs": {"result": {"terms": {"field": "status"}}}}
...
def get_text():
    ...
    if url_file is None:
        return jsonify({"success": False, "content": "No job file specified"})
    ...
    if r.status_code == 404:
        success = False
        content = (
            '<font color="red"><b>Got 404 trying to access:<br/><br/>%s</b><br/><br/>'
            % url_file
        )
        content += "Check that WebDAV has been configured on worker node.<br/><br/>"
        content += (
            "If WebDAV is up and running on the worker node, then this job's<br/>"
        )
    ...
    return jsonify({"success": success, "content": content})
